    ...
    public static function run($params = null)
    {
		...
		if ($res['status'] == 0) {
			try {
			    ...
			    /*身份证校验*/
			    if(!empty($userExists['idcard'])){
			        if($userExists['idcard'] !=$res['data']['cert_no']){
			            throw new Zeed_Exception('和已登记身份证不符');
			        }
			    } else{
			       $certExists = Cas_Model_User::instance()->fetchByWhere( "idcard= '{$res['data']['cert_no']}'");
			       if ($certExists) {
			           throw new Zeed_Exception('此身份证已被其他用户绑定');
			       }
			    }

			     /*绑定银行卡信息*/
			     $bank_info = Cas_Model_Bank::instance()->fetchByWhere("bank_no='{$res['data']['card_no']}' and is_use=1 and is_del=0 and type=1");
			    ...
			    $bank['bank_name'] = $params['bank_name'];//银行名称
			    ...
			    $bank['is_use'] = 0 ;//是否使用
			    $bank['type'] = 2 ;//支付平台 1:融宝 2：富友
			    $bank['phonebankcard'] = $params['phone'];//银行卡绑卡手机号
			    $bank['cert_no'] = $params['cert_no'];//身份证号
			    ...
			    //修改用户信息
			    $id = Cas_Model_User::instance()->update(array('username'=>$params['owner'],'bank_id'=>$bankid),"userid= '{$bank['userid']}'");
...
// 			    $data['bind_id'] = $bank['bind_id'];//绑卡id
			    $data['bank_name'] = $res['data']->bank_name;//银行卡名
